...
<head>
        ...
        if (empty($_SERVER['QUERY_STRING'])) echo "Главная"; else
        {
         $titles = array( 
            ...
            "detail_se" => "Детали с поиска за ".substr($_GET['detail_se'],6,2).".".substr($_GET['detail_se'],4,2).".".substr($_GET['detail_se'],0,4),
            "detail_other" => "Детали c других сайтов за ".substr($_GET['detail_other'],6,2).".".substr($_GET['detail_other'],4,2).".".substr($_GET['detail_other'],0,4),
            "detail_ho" => "Детали хостов за ".substr($_GET['detail_ho'],6,2).".".substr($_GET['detail_ho'],4,2).".".substr($_GET['detail_ho'],0,4),
            "detail_hi" => "Детали хитов за ".substr($_GET['detail_hi'],6,2).".".substr($_GET['detail_hi'],4,2).".".substr($_GET['detail_hi'],0,4),
            "detail" => "Детали за ".substr($_GET['detail'],6,2).".".substr($_GET['detail'],4,2).".".substr($_GET['detail'],0,4),
            "fix" => "fix-ы за ".substr($_GET['fix'],6,2).".".substr($_GET['fix'],4,2).".".substr($_GET['fix'],0,4),
...
<table align=center width=250 cellpadding=5 cellspacing=1 border=0><tr class=h><td width=250>Авторизация</td></tr>
<tr class="s1">
<td align=left>
<form style='margin:0px;padding:0px;' action="" method="post">
<center>
<div style="width:80px;float:left;vertical-align:middle;height:19px;padding-top:3px;"><span style="margin-left:16px;"><b>Логин:</b></span></div><div style="float:left;vertical-align:middle;height:19px;"><input type="text" name="alogin" style="width:134px;"></div>
<div style="clear:both;height:0px;"></div>
<div style="width:80px;float:left;vertical-align:middle;height:19px;padding-top:3px;"><span style="margin-left:16px;"><b>Пароль:</b></span></div><div style="float:left;vertical-align:middle;height:19px;"><input type="Password" name="apass" style="width:134px;"></div>
<div style="clear:both;vertical-align:middle;height:19px;padding-top:1px;padding-bottom:3px;"><input name="save" type="checkbox" value='1' checked style='vertical-align:middle;'> <span style='vertical-align:middle;'>Запомнить меня</span></div>
<div style="padding:3px;"><input type="submit" value="   Вход   "></div>
<? if ($err == 1) { echo "<font color=red>Неверные логин или пароль</font>"; 
...
<div align=center style="font-family: Tahoma, 'Lucida Grande'; margin-top: 10px; font-size: 10px; font-weight: bold; color: #696969;">2013 &copy; Copyright by <a href="http://www.fri-stats.com">www.fri-stats.com</a> &nbsp;<a href="http://vk.com/i_fri" title="Андрей Фри" target=_blank><img src="flags/vk.png" border=0 style="vertical-align:middle"></a></div>
</body>
</html>
<? 
exit;
...
function se_yandex($ref)
{
 ...
 $url = strip_tags($url);
 if (substr_count("$url","Г") > 2) $url = iconv("UTF-8", "CP1252", $url);
 if (substr_count("$url","Р") > 2) $url = iconv("UTF-8", "CP1251", $url);
...
function se_google($ref)
{
 ...
 if (is_ruUTF8(trim($match1[1]))) $match1[1] = iconv("UTF-8", "CP1251", trim($match1[1]));
 if (substr_count($match1[1],"Р") > 2) $match1[1] = iconv("UTF-8", "CP1251", trim($match1[1]));
 preg_match("/[?&]+".$sw2."([^&]*)/i", $url."&", $match2); 
 if (is_ruUTF8(trim($match2[1]))) $match2[1] = iconv("UTF-8", "CP1251", trim($match2[1]));
 if (substr_count($match2[1],"Р") > 2) $match2[1] = iconv("UTF-8", "CP1251", trim($match2[1]));
 if ($match2[1] == $match1[1]) return array($engine, $match1[1]);
 if (!empty($match2[1])) return array($engine, ($match2[1]." + ".$match1[1]));
 else return array($engine, $match1[1]);
}
...
function se_rambler($ref)
{
 ...
 $engine = "R";
 $url = urldecode($ref);
 if (stristr($url, "btnG=оБКФЙ!")) $url = convert_cyr_string($url, "k", "w");
 $url = stripslashes($url);
 $url = strip_tags($url);
 preg_match("/[?&]+".$sw."([^&]*)/i", $url."&", $match1); 
...
function echo_se($engine)
{
 global $se_n;
 switch ($engine) {
   case "Y": echo "<b><font color=#FF0000>Я</font>ndex</b>"; break;
...
<tr class="s1">
<td align=left>
<br><div align="center">Доступ закрыт!</div><br>
</td>
</tr>
</table>
<div align=center><br><a href=?logout><b>Выйти</b></a></div><br><style type="text/css"> body { cursor: auto; } </style>
</body>
</html>
<?
exit;
...
<tr class=h>
<td width=100% valign=middle><form style="margin:0px;padding:0px;" action="" method="get"><span style="vertical-align: middle;">Фильтр: &nbsp;<font color=#de3163>с</font></span> <input type=text name="s_date" style="width: <?=(APPLE==1)?"80":"70";?>px; font-family: Tahoma, 'Lucida Grande'; font-size: 11px; vertical-align: middle;" value="<? echo dtconv($s_date); ?>">
<span style="vertical-align: middle;"><font color=#de3163>до</font></span> <input type=text name="f_date" style="width: <?=(APPLE==1)?"80":"70";?>px; font-family: Tahoma, 'Lucida Grande'; font-size: 11px; vertical-align: middle;" value="<? echo dtconv($f_date); ?>">
<? if (!in_array($_GET['top'], array(17,11,8)) and ($_GET['top'] != 4 or isset($_GET['engin']))) { ?>
&nbsp;<span style="vertical-align: middle;">кол-во поз.</span> <input type=text name="pos" style="width: 30px; font-family: Tahoma, 'Lucida Grande'; font-size: 11px; vertical-align: middle;" value="<? if ($_GET['pos'] == 99999) echo ""; else echo $_GET['pos']; ?>">
<? } ?>
<? if (!in_array($_GET['top'], array(5,6,7,13,10,14,16)) or ($_GET['top'] == 10 and !isset($_GET['domen']))) { ?>
&nbsp;<span style="vertical-align: middle;">сорт.</span> <select name="sort" style="font-family: Tahoma, 'Lucida Grande'; font-size: 11px; vertical-align: middle;">
   <option value="ho" <? if ($_GET['sort'] == "ho" or $_GET['sort'] != "hi") echo "selected"; ?>>Хосты</option>
   <option value="hi" <? if ($_GET['sort'] == "hi") echo "selected"; ?>>Хиты</option>
 </select>
<? } ?>
...
&nbsp;<span style="vertical-align: middle;">строка</span> <input type=text name="str_f" style="width: <?=(APPLE==1)?"140":"160";?>px; font-family: Tahoma, 'Lucida Grande'; font-size: 11px; vertical-align: middle;" value="<? if ($_GET['str_f']) echo $_GET['str_f']; ?>">
<? } ?>
<input style="width: <?=(APPLE==1)?"60":"72";?>px; font-family: Tahoma, 'Lucida Grande'; font-size: 11px; padding: 1px; vertical-align: middle;" type=submit value="<?=(APPLE==1)?"Показ!":"Показать!";?>">
<? if (APPLE!=1) { ?>
...
if ($group <> "true")
{
 ...
 while ($row = mysql_fetch_row($r))
 {
  ...
  if ($row[5] != "")
  {
   echo "<br>Язык: ".(!empty($lang[strtoupper($row[5])])?$lang[strtoupper($row[5])]:"<font color=grey>неизвестно</font>"); 
...
else
{
 ...
 foreach($row as $ip => $val) 
 {
  ...
  foreach($val as $k => $rw) 
  {
      ...
      if ($rw[4] != "")
      {
       echo "<br>Язык: ".(!empty($lang[strtoupper($rw[4])])?$lang[strtoupper($rw[4])]:"<font color=grey>неизвестно</font>"); 
...
if (isset($detail_se))
{
 ...
 foreach($i1 as $id => $row) 
 {
  ...
  if ($row[4] != "")
  {
   echo "<br>Язык: ".(!empty($lang[strtoupper($row[4])])?$lang[strtoupper($row[4])]:"<font color=grey>неизвестно</font>");
...
if (isset($conf) || isset($f_del) || isset($grp) || isset($erd) || isset($z) || isset($h) || isset($utf8))
{
 ...
 echo "<input style=\"width: ".((APPLE==1)?"40":"32")."px;\" type=submit value=\"Да!\"></form>";
      if (file_exists("errors.dat") and filesize("errors.dat") > 2) { $errc = "есть!"; $type = "submit"; } else { $errc = "нет"; $type = "hidden"; }
 ...
 echo "<br><input style=\"width: ".((APPLE==1)?"140":"120")."px;\" name=erd type=".$type." value=\"Очистить errors.dat!\">"; } echo "</form>";
 echo "</td></tr>";
?>
<script type="text/javascript">
function selh() {
...
{  
 ...
 foreach($row as $req => $val) 
 {
  ...
  foreach($val as $k => $rw) 
  {
      ...
      if ($rw[6] != "")
      {
       echo "<br>Язык: ".(!empty($lang[strtoupper($rw[6])])?$lang[strtoupper($rw[6])]:"<font color=grey>неизвестно</font>"); 
 ...
 while ($row = mysql_fetch_row($res))
 {
   ...
   if ($row[7] != "") echo "<br>Язык: ".(!empty($lang[strtoupper($row[7])])?$lang[strtoupper($row[7])]:"<font color=grey>неизвестно</font>");
...
if (isset($top))
{
 ...
 switch ($top)
 {
   ...
   if (!isset($othmas)) { echo "<center>Нет данных</center>"; break; }
   ...
   if (isset($engin) or $top == 1)
   {
       ...
       while ($row = mysql_fetch_row($res))
       {
         ...
         if (is_array($refer))
         {
          ...
          if (empty($query)) $query = "<font color=grey>неизвестно</font>"; $qmas[] = strtolower($query); if ($pages == 1) $pmas[strtolower($query)][] = $row[1]; } } 
          else { if (empty($query)) $query = "<font color=grey>неизвестно</font>"; $qmas[] = strtolower($query); if ($pages == 1) $pmas[strtolower($query)][] = $row[1]; }
         }
       }
       if (!isset($qmas)) { echo "<center>Нет данных</center>"; break; }
       ...
       echo_se($engin);
       echo "&nbsp;&nbsp;</span>"; }
       echo (($pages<>1)?" <a class=e href=\"?".str_replace(stristr($_SERVER['QUERY_STRING'],'&pages'),"",$_SERVER['QUERY_STRING'])."&pages=1\">+</a>":"")."</td>".(($pages<>1)?"":"<td width=50%>Страницы <a class=e href=\"?".str_replace(stristr($_SERVER['QUERY_STRING'],'&pages'),"",$_SERVER['QUERY_STRING'])."&pages=0\">&ndash;</a></td>")."<td width=50>".(($sort == "hi")?"Хиты":"Хосты")."</td><td width=100>График</td><td width=50>%</td></tr>";
   ...
   if (!isset($enmas)) { echo "<center>Нет данных</center>"; break; }
   ...
   if (!isset($tmas)) { echo "<center>Нет данных</center>"; break; }
   arsort($tmas);
   $mmx = max($tmas);
   $cnt = array_sum($tmas);
   ...
   if (!isset($dmas)) { echo "<center>Нет данных</center>"; break; }
   arsort($dmas);
   $mmx = max($dmas);
   $cnt = array_sum($dmas);
   ...
   if (!isset($bmas)) { echo "<center>Нет данных</center>"; break; }
   arsort($bmas);
   $mmx = max($bmas);
   $cnt = array_sum($bmas);
...
include "graph_days.php";
   ...
   $sline .= "<td valign=top>".$f_months."<img id=map_months src=".$login."_months.png height=98 width=288 border=0 USEMAP=\"#map_months\"></td>
        ...
        <td width=150 valign=top>
                ...
                <tr><td valign=middle>
                <input style=\"vertical-align: middle; width: ".((stristr($u,"MSIE 10") or (stristr($u,"firefox") and stristr($u,"mac")))?"32":((APPLE==1)?"38":"34"))."px; font-family: Verdana, 'Lucida Grande'; font-size: ".((APPLE==1)?"10":"9")."px; font-weight: bold;\" type=submit value=\"Ok\"> ".((APPLE==1)?"":"&nbsp;")."&nbsp;<span style=vertical-align:middle>Дни:</span>&nbsp;
